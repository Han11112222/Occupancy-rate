...
# =============== 기본 세팅 ===============
import os, platform, logging, warnings
import numpy as np
import pandas as pd
...
def load_df_from_path_or_buffer(path: str, buffer):
    if buffer is not None:
        df_local = pd.read_excel(buffer, sheet_name="data")
    else:
        if not os.path.exists(path):
            st.error(f"엑셀 경로를 확인하세요: {path}")
            return pd.DataFrame()
...
if load_way == "Repo 내 파일 사용":
    ...
    if uploaded_file is not None:
        df = load_df_from_path_or_buffer(None, uploaded_file)
    else:
        df = pd.DataFrame()

# =============== 공통 유틸 ===============
def ensure_start_index(_df):
    ...
    _df["입주시작월"] = _df.apply(
        ...
        if pd.notna(r["입주시작index"]) and pd.notna(r["공급승인일자"]) else pd.NaT,
        axis=1
    )
    return month_cols
...
def plot_yearly_avg_occupancy_with_plan(start_date, end_date, min_units=0):
    ...
    plan_x = list(range(1, MAX_M+1)); plan_y = [PLAN[i]/100 for i in plan_x]
...
def recent2y_top_at_5m(end_date, top_n=10, min_units=0):
    ...
    if eligible.empty:
        st.info("최근 2년 코호트에서 5개월차까지 도달한 단지가 없어.")
        return pd.DataFrame()
...
def cohort2025_progress(end_date, min_units=0, MAX_M=9):
    ...
    if cohort.empty:
        st.info("2025년 입주시작 단지(조건 충족)가 없어.")
        return pd.DataFrame()
    ...
    if out_df["선택일기준_누적입주율"].notna().any():
        ...
        st.info("선택일 기준 누적입주율을 계산할 수 있는 단지가 없어.")
    return out_df
...
def underperformers_vs_plan(end_date, min_units=0, MAX_M=9, top_n=15):
    ...
    out = out[["아파트명","세대수","입주시작월","경과개월(선택일기준)",
    ...
    for y, (a_units, p_units, lack) in enumerate(zip(
